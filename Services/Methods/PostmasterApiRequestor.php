...
class PostmasterApiRequestor
{
	...
	public function request($meth, $url, $params=null, $headers=null)
	{
		...
		throw new PostmasterError("Invalid response body from API: $rbody (HTTP response code was $rcode)", $rcode, $rbody);
		}
		}
		...
		if ($rcode == 400) {
		throw new Error\InvalidDataError($msg, $rbody, $rcode, $resp);
		} else if ($rcode == 401) {
		throw new Error\AuthenticationError($msg, $rbody, $rcode, $resp);
		} else if ($rcode == 403) {
		throw new Error\PermissionError($msg, $rbody, $rcode, $resp);
		}
		throw new Error\APIError($msg, $rbody, $rcode, $resp);
		}
		return $resp;
	}
	...
	private function _curlRequest($meth, $absUrl, $headers, $params, $apiKey)
	{
		...
		throw new PostmasterError("Unrecognized method $meth");
		}
