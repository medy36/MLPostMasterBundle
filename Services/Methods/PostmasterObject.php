...
* Commons
*/
class PostmasterObject implements \ArrayAccess
{
	...
	'PostmasterShipment.to' => 'PostmasterAddress',
	'PostmasterShipment.from' => 'PostmasterAddress',
	'PostmasterShipment.package' => 'PostmasterPackage',
	'PostmasterTracking.last_update' => 'DateTime',
	'PostmasterTrackingHistory.timestamp' => 'DateTime',
	'PostmasterTransitTime.delivery_timestamp' => 'DateTime',
	'PostmasterTracking.data' => 'PostmasterExternalPackage'
	);

	// which keys should be converted to list of Postmaster_Objects
	protected static $obj_list_keys = array(
	'PostmasterAddressValidation.addresses' => 'PostmasterAddress',
	'PostmasterShipment.packages' => 'PostmasterPackage',
	'PostmasterTracking.history' => 'PostmasterTrackingHistory'
	);
	...
	if (strpos($class,'ML\PostMasterBundle\Services\Methods') !== false) {
    	
    	$classname =  $class;

	}else{
		
		$classname = "\ML\PostMasterBundle\Services\Methods\\" . $class;

	}
	
	$obj = new $classname(isset($values['id']) ? $values['id'] : null);
	...
	*/
	public function __toArray()
	{
		...
		foreach ($this->_values as $k => $v) {
			$fullKey = get_class($this) . '.' . $k;
			if ($v instanceof PostmasterObject) {
			// convert to array
